...
创建之后，句柄值可以移动。它可以从 child scope 返回，可以传给另一段 routine，也可以被
回调代码保留。移动这个值，不会移动它的归属。
...
归属在 `future(...)` 调用发生时确定。它运行在 `panelScope` 内部，所以 `panelScope`
...
- `resource(...)` 会创建用于提供值的 future，并启动挂在当前 scope 上直到释放前的
  provider 工作。

使用句柄的调用不会选择新的归属。观察或完成 future 的调用，例如 `wait(...)` 和
`settle(...)`，作用于已有的 future。发送、接收或关闭 channel 的调用，作用于已有的
channel。把 `AbortSignal` 传给外部 API，通过 `promisify(...)` 暴露 future，或保留
`resource(...)` 提供的值，也是在使用已经绑定到某个 scope 的关系。归属仍然由创建句柄时所在的
scope 决定。
