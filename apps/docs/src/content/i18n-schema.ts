import { z } from "astro/zod";

export const explorerUiI18nSchema = z.object({
  "docs.404.body": z.string(),
  "docs.404.description": z.string(),
  "docs.404.hero.backHome": z.string(),
  "docs.404.hero.explorer": z.string(),
  "docs.404.hero.tagline": z.string(),
  "docs.404.title": z.string(),
  "docs.home.body": z.string(),
  "docs.home.description": z.string(),
  "docs.home.hero.explorer": z.string(),
  "docs.home.hero.guides": z.string(),
  "docs.home.hero.tagline": z.string(),
  "docs.home.title": z.string(),
  "explorer.areas.dependencies.description": z.string(),
  "explorer.areas.dependencies.title": z.string(),
  "explorer.areas.states.description": z.string(),
  "explorer.areas.states.title": z.string(),
  "explorer.areas.timeline.description": z.string(),
  "explorer.areas.timeline.title": z.string(),
  "explorer.examples.fork-join.description": z.string(),
  "explorer.examples.fork-join.guide.fork": z.string(),
  "explorer.examples.fork-join.guide.join": z.string(),
  "explorer.examples.fork-join.title": z.string(),
  "explorer.examples.future-settlement.description": z.string(),
  "explorer.examples.future-settlement.guide.settle": z.string(),
  "explorer.examples.future-settlement.guide.wait": z.string(),
  "explorer.examples.future-settlement.title": z.string(),
  "explorer.examples.single-spawn.description": z.string(),
  "explorer.examples.single-spawn.guide.parent": z.string(),
  "explorer.examples.single-spawn.guide.spawn": z.string(),
  "explorer.examples.single-spawn.title": z.string(),
  "explorer.home.intro": z.string(),
  "explorer.home.title": z.string(),
  "explorer.note.body": z.string(),
  "explorer.note.label": z.string(),
  "explorer.replay.follow": z.string(),
  "explorer.replay.manual": z.string(),
  "explorer.shell.backToDocs": z.string(),
  "explorer.shell.eyebrow": z.string(),
});

export type ExplorerUiStrings = z.infer<typeof explorerUiI18nSchema>;
