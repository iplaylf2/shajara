...
- `Interpreter` 负责解释 wisp、组织后续 `resonate` / `primeResonate` 的推进，并推动 process 继续执行。
...
这里固定一条设计约束：`RuntimeScope` 读取 process 终态时，通过 keeper 暴露的 `RuntimeProcessKeeperState` 分支进行读取。当前 keeper 侧已冻结的状态载荷只有 completed result 与 failed failure。runner 侧则进一步按 interpreting / resonating / waiting / closed 分支冻结解释所需能力。
