...
6. runtime 的 `operations-kit` 已收敛到执行入口桥接（`execution-as-promise`、`launch-runtime-blueprint`），不再承载 `until/receive` 状态抽象。Evidence: `packages/runtime/src/operations-kit/execution-as-promise.ts`, `packages/runtime/src/operations-kit/launch-runtime-blueprint.ts`, `packages/runtime/src/operations`。
