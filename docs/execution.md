...
| 治理角色 handler 契约     | `SchedulerScopeSpec` 与 `ReaperScopeSpec` 已改为必填 `*Config` 并显式携带 handler；具体签名单源见 `docs/semantics.md`，实现见 `packages/kernel/src/scopes/scheduler.ts`、`packages/kernel/src/scopes/reaper.ts`。 |
...
| 治理 handler 执行接线      | `scheduler/reaper` 的 spec-level handler 形状已确定；executor 侧触发、校验与消费返回值的执行接线待实现。                                              |
