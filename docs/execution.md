...
2. `kernel` 执行入口契约已收敛到 `rootScope + launch + post + terminate`，并通过 `ExecutionScope.result.onResult(...)` 暴露三态结果（`success | failure | interruption`）。Evidence: `packages/kernel/src/executor.ts`, `packages/kernel/src/index.ts`, `packages/runtime/src/operations-kit/execution-as-promise.ts`。
...
5. runtime 宿主操作与结果收敛形状已落地：`run`、`createScope`、`action`、`sleep`、`until` 的调用协议与错误映射完整，`failure/interruption` 分别映射到 `RuntimeScopeFailedError/RuntimeScopeInterruptedError`。Evidence: `packages/runtime/src/operations/run.ts`, `packages/runtime/src/operations/create-scope.ts`, `packages/runtime/src/operations/action.ts`, `packages/runtime/src/operations/sleep.ts`, `packages/runtime/src/operations/until.ts`, `packages/runtime/src/operations-kit/execution-as-promise.ts`, `packages/runtime/src/errors`。
6. 已验证 `@khora/runtime` 与 `@khora/kernel` typecheck 通过。Evidence: workspace 命令 `yarn workspace @khora/runtime typecheck`、`yarn workspace @khora/kernel typecheck`。
...
Impact: runtime 已统一 `ExecutionResult -> Promise` 收敛策略，`failure/interruption` 分支异常类型已固定，可作为后续实现与验证阶段的稳定边界。Evidence: `packages/runtime/src/operations-kit/execution-as-promise.ts`, `packages/runtime/src/errors/runtime-scope-failed.ts`, `packages/runtime/src/errors/runtime-scope-interrupted.ts`。
...
| B4 runtime 执行入口闭环（run）                    | In Progress | `run`/`scope.run` 已按目标契约接线到 `launch + executionAsPromise`，但运行依赖 B2。        | `packages/runtime/src/operations/run.ts`, `packages/runtime/src/operations/create-scope.ts`, `packages/runtime/src/operations-kit/execution-as-promise.ts`, `packages/kernel/src/executor.ts`                                  |
