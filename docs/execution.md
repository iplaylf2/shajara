...
6. `self/spawn` 相关字段命名已与语义文档对齐：`SelfDescriptor` 使用 `scopeId/processId`，`SpawnDescriptor` 使用 `scopeId/rootProcessId`，runtime 侧 `sleep/until` 的宿主投递入口已同步消费新字段。Evidence: `packages/kernel/src/syscalls/self.ts`, `packages/kernel/src/syscalls/spawn.ts`, `packages/runtime/src/operations/sleep.ts`, `packages/runtime/src/operations/until.ts`, `docs/semantics.md`。
