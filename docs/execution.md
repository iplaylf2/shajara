...
2. `kernel` 执行入口契约维持“显式 scope 锚点 + 单一 launch + post(scope, value)”；`rootScope` 为只读锚点值（非方法），`launch` 目标为 `RootScopeRef | ExecutionScopeRef`，`post` 目标为 `ScopeRef`，`terminate` 目标为 `ExecutionScopeRef`；`Executor` 不再暴露 `createScope`。`ExecutionScope` 暴露 `result: ExecutionFuture<T>` 与 `state()`，`ExecutionResult<T>` 采用三态 sum type（`success | failure | interruption`）。Evidence: `packages/kernel/src/executor.ts`, `packages/kernel/src/index.ts`, `packages/runtime/src/operations-kit/execution-as-promise.ts`。
...
4. runtime 语义桥已闭环：`lowerPlan` 与 `liftPlan` 都已落地，`then/terminate` 路径按 generator 协议推进；桥接已收敛到 `lowerPlan` 单入口，primitive tuple 降解约束集中在 `lower-runtime-blueprints` 边界内。Evidence: `packages/runtime/src/adapter/plan-lower.ts`, `packages/runtime/src/adapter/plan-lift.ts`, `packages/runtime/src/primitives-kit/lower-runtime-blueprints.ts`, `packages/runtime/src/primitives/all.ts`, `packages/runtime/src/primitives/race.ts`。
5. runtime 宿主操作主路径已接通：`until` 采用 `scoped + receive + post`，`sleep` 采用 `scoped + try/finally(clearTimeout) + receive`，`action` 采用 `spawn + receive + post` 返回宿主结算句柄；`createScope` 通过 `launch(rootScope, suspend)` 创建托管 scope，并以 `state === open` 作为 `terminate` 触发门闩；相关 `scope/spawn/self` 描述类型直接消费 kernel 导出类型，不再由 runtime 重新包装。Evidence: `packages/runtime/src/operations/until.ts`, `packages/runtime/src/operations/sleep.ts`, `packages/runtime/src/operations/action.ts`, `packages/runtime/src/operations/create-scope.ts`, `packages/runtime/src/primitives/self.ts`, `packages/runtime/src/primitives/spawn.ts`。
6. runtime 的 `operations-kit` 已收敛到执行结果桥接（`execution-as-promise`），不再承载 `until/receive` 状态抽象。Evidence: `packages/runtime/src/operations-kit/execution-as-promise.ts`, `packages/runtime/src/operations`。
...
Impact: `kernel` 执行入口从“运行器细节暴露”收敛为“结果收敛契约暴露”：runtime 只消费 `ensureExecutor().rootScope`、`ensureExecutor().launch(scope, ...)` 与 `execution.result.onResult(...)`，不承担执行器创建与推进职责，避免跨层职责漂移。Evidence: `packages/kernel/src/executor.ts`, `packages/kernel/src/index.ts`, `packages/runtime/src/operations/run.ts`, `packages/runtime/src/operations-kit/execution-as-promise.ts`。
...
Impact: `runtime.run` 与 `runtime.createScope().run` 走同一个 kernel 调用形状：`launch(scope, blueprint)`；差异只在 scope 来源（`rootScope` 或托管 `scope.ref`）。runtime 不再维护并行入口语义。Evidence: `packages/kernel/src/executor.ts`, `packages/runtime/src/operations/run.ts`, `packages/runtime/src/operations/create-scope.ts`。
...
### 4.14 增量：执行入口契约命名与语义纠偏（对话回顾固化）

Impact: 本轮对话明确了“先定语义，再定命名，再实现适配”的约束，避免在 `createScope` 或 runtime 侧用额外抽象掩盖 kernel 契约问题。已固化的决议如下：

1. `ExecutionScope` 是统一执行句柄，不再并行维护同语义的 `ExecutionHandle/ExecutionScopeHandle` 分裂模型。
2. `ExecutionScope` 不包含 `terminate` 方法；作用域终止统一由 `executor.terminate(scopeRef)` 发起。
3. `launch()` 是唯一执行句柄创建入口：`launch(rootScope, suspend)` 可生成托管 scope（`ExecutionScope<never>`），`launch(scope, blueprint)` 返回业务执行句柄（`ExecutionScope<ReturnValue>`）。
4. `rootScope` 是只读值，不是方法；其类型为专用 `RootScopeRef`。
5. `ExecutionResult<T>` 采用三态 sum type（`success | failure | interruption`），kernel 不把失败强制异常化。
6. `ExecutionFuture<T>` 与 `PromiseLike` 解耦；当前采用 `onResult(listener)` 暴露一次性结算观察接口。
7. runtime 侧 `execution-as-promise` 通过 `ExecutionResult` 做分支收敛，其中 `interruption` 映射为 `RuntimeScopeInterruptedError`，`failure` 映射为 `RuntimeScopeFailedError`。

Evidence: `packages/kernel/src/executor.ts`, `packages/kernel/src/index.ts`, `packages/runtime/src/operations-kit/execution-as-promise.ts`, `packages/runtime/src/errors/runtime-scope-interrupted.ts`, `packages/runtime/src/operations/create-scope.ts`。

### 4.15 增量：执行结果收敛入口与失败异常语义收敛

Impact: runtime 将 `ExecutionScope -> Promise` 的收敛入口统一命名为 `executionAsPromise`，并在 `run/scope.run/scope.closed` 复用同一分支收敛逻辑；`failure` 统一映射到 `RuntimeScopeFailedError`，`interruption` 统一映射到 `RuntimeScopeInterruptedError`。同时，`halt` 去重从本地 Promise 缓存收敛为 `scope.state()` 门闩。Evidence: `packages/runtime/src/operations-kit/execution-as-promise.ts`, `packages/runtime/src/errors/runtime-scope-failed.ts`, `packages/runtime/src/operations/run.ts`, `packages/runtime/src/operations/create-scope.ts`。

### 4.16 增量：runtime primitive 命名与边界约束收敛

Impact: primitive tuple 降解逻辑命名从 `factories` 收敛为 `blueprints`，并将 tuple 约束类型放回约束提出点（`lower-runtime-blueprints`）而非 runtime 对外契约文件；`race` 直接使用 kernel `RaceResult` 形状，避免 runtime 侧同语义别名包装。Evidence: `packages/runtime/src/primitives-kit/lower-runtime-blueprints.ts`, `packages/runtime/src/primitives/all.ts`, `packages/runtime/src/primitives/race.ts`, `packages/runtime/src/contracts.ts`。
...
| B2 kernel 执行入口契约与最小实现                  | In Progress | 执行入口契约维持 `rootScope` + `launch(scope, blueprint)` + `post(scope, value)` + `terminate(scope)`；目标引用类型收敛为 `RootScopeRef/ExecutionScopeRef/ScopeRef/SpawnRef/SelfDescriptor`，实现仍占位。 | `packages/kernel/src/executor.ts`, `packages/kernel/src/index.ts`, `packages/kernel/src/contracts.ts`, `docs/semantics.md`                                                                                                       |
| B3 runtime 语义桥闭环（lower/lift）               | Completed   | `RuntimePlan -> Plan` 与 `Plan -> RuntimePlan` 适配已落地，`then/terminate` 双路径已接入 generator 协议，且桥接入口收敛为 `lowerPlan`。                                                                   | `packages/runtime/src/adapter/plan-lower.ts`, `packages/runtime/src/adapter/plan-lift.ts`, `packages/runtime/src/primitives-kit/lower-runtime-blueprints.ts`, `docs/runtime.md`                                                  |
