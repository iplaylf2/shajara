...
- `lift` 固定表示 `kernel -> runtime` 的语义适配，即把 `Plan<T>` 嵌入为可被 generator `yield*` 消费的 `RuntimePlan<T>`。
- `lower` 固定表示 `runtime -> kernel` 的语义适配，即把 runtime 侧编排表达降解为 kernel 可执行表达。
- “上/下”是按语义层级而非实现位置命名：`kernel` 是执行语义单源，`runtime` 是用户编排与宿主边界表达层。
- 因此从执行语义到用户表达称为 `lift`；从用户表达回到执行语义称为 `lower`。
- 该命名约束的目的，是避免把 runtime 误当作第二语义源，保持 `then/terminate` 的执行真相只在 kernel 收敛。
