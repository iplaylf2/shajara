...
- 失败 → 抛出 `Error`；结构性失败表现为 `ShajaraError` 子类，用户侧抛出的外部异常会尽量保留原始实例。
...
| `resumable` | `resumable(ritual) → RiteFuture<T>`          | 声明一段可由外围 `guard` 恢复的计算，并返回该 entry process 的 future。                                                                   |
| `guard`     | `guard(entry, recover) → RiteFuture<void>`   | 为 `entry` 对应的子树建立恢复边界；该子树内 `resumable` 上送的失败由 `recover(error: Error)` 处理，并返回该子树入口 scope 的完成 future。 |
