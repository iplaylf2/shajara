...
宿主 API 用于启动与驱动运行时，并承接与宿主世界的交互边界。

- 创建运行时实例
- 启动一段 `Flow` 并得到宿主侧结果
- 将宿主侧事件投递到某个运行中的 `Scope`

宿主 API 不参与并发编排。并发结构由编排原语在 `Flow` 内完成。

### 1.2 编排原语 API

编排原语用于在 `Flow` 内描述并发结构。
...
启动一段 `Flow` 并在宿主侧等待其结果。

- 负责创建根 `Scope` 与根 `Process`
- 负责启动推进循环
- 负责将根 `Flow` 的完成映射为宿主侧结果
