...
- kernel primitive 通过显式代数通道（`Either<KhoraFailure, T>`）表达失败，不依赖宿主 `throw`。
- runtime 在 primitive 适配边界统一解包 `Either`，将 `Left` 收敛为 `RuntimeKhoraError` 抛出。
- `KhoraFailure` 由 kernel 定义，runtime 包装为 `RuntimeKhoraError`（`Error` 子类），不向用户暴露 kernel 失败类型。
