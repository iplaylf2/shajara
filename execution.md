...
- Build: 当前阶段。聚焦 kernel/runtime 适配方向契约固化后推进执行闭环。
- Prove: 在适配方向落地后补执行语义验证与回归。
...
- runtime 计划提升入口已收敛到 `liftPlan`，并从 `plan-runtime.ts` 重命名为 `plan-lift.ts`。Evidence: `packages/runtime/src/adapter/plan-lift.ts:1`, `packages/runtime/src/adapter/plan-lift.ts:4`
...
- 术语方向已在设计文档固化：`lift = kernel -> runtime`、`lower = runtime -> kernel`。Evidence: `docs/runtime.md:89`, `docs/api.md:27`, `docs/design-constraints.md:16`
- runtime 侧跨包适配已以 `lowerPlan` 为主入口，`lowerBlueprint` 仅作为蓝图入口的薄包装。Evidence: `packages/runtime/src/adapter/plan-lower.ts:4`, `packages/runtime/src/primitives/all.ts:11`, `packages/runtime/src/operations/run.ts:7`
...
### 4.2 runtime 方向契约已固化，代码尚未完成对齐

- Impact: 术语歧义收敛为单一方向定义，后续代码可按 `lift/lower` 对偶语义重构。
- Evidence: `docs/runtime.md:89`, `docs/api.md:27`, `docs/design-constraints.md:16`

### 4.3 主阻力仍是 lower/run/createScope/until 未实现

- Impact: 方向契约文档已定后，下一步应集中在 `runtime -> kernel` 降解路径与执行驱动闭环，而非继续停留在 bridge 比喻层。
- Evidence: `packages/runtime/src/adapter/plan-lower.ts:7`, `packages/runtime/src/operations/run.ts:8`, `packages/runtime/src/operations/create-scope.ts:17`, `packages/runtime/src/operations/until.ts:8`
...
- Evidence: `packages/runtime/src/contracts.ts:1`, `packages/runtime/src/adapter/plan-lower.ts:1`, `packages/runtime/src/adapter/plan-lift.ts:1`
...
### 5.3 Slice B3：lower 执行闭环

- Status: **In Progress**
- Output: `run/createScope/until` 与 `runtime -> kernel` 降解路径仍为占位，尚未形成执行闭环。
- Evidence: `packages/runtime/src/operations/run.ts:8`, `packages/runtime/src/operations/create-scope.ts:17`, `packages/runtime/src/operations/until.ts:8`, `packages/runtime/src/adapter/plan-lower.ts:7`
- Next: 在 `operations/run` 与适配边界中落地 `lower` 执行映射，并补失败传播路径。
...
- Evidence: `packages/kernel/src/primitives.ts:22`, `packages/runtime/src/primitives/cede.ts:5`, `packages/runtime/src/primitives/race.ts:22`, `packages/runtime/src/adapter/plan-lift.ts:4`
- Next: 在 `liftPlan` 实现阶段显式落地 `then/terminate` 对 generator 推进与 `try...finally` 的对齐策略。

## 6. 后续阶段方向

- Prove: 补 `lift/lower` 方向行为验证，覆盖正常返回、异常传播、终止路径。
